      ...
      {teams.map((t) => (
        <Credenza key={t.key}>
          <CredenzaTrigger asChild>
            <Card className="my-0 mb-1 flex h-[150px] cursor-pointer items-center gap-4 rounded-lg bg-background p-4">
              ...
              {(() => {
                ...
                return maybeImage === undefined ? null : (
                  <img
                    ...
                    className="content-visibility-auto h-full w-1/3 rounded-lg border-4 border-neutral-400 object-cover"
                    loading="lazy"
                  />
                );
              })()}
...
function getTeamRobotPic(media: Media[]): string | undefined {
  ...
  function fixInstagramOnDev(url: string | undefined, type: string) {
    if (type === 'instagram-image') {
      return `https://www.thebluealliance.com/${url}`;
    }
  ...
  return maybePreferred
    ? fixInstagramOnDev(maybePreferred.direct_url, maybePreferred.type)
    : fixInstagramOnDev(pics[0].direct_url, pics[0].type);
}
