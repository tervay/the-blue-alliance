...
InsightLeaderboardDict = NewType("InsightLeaderboardDict", Dict)
...
class InsightConverter(ConverterBase):
    ...
    @classmethod
    def insight_dict_to_api_leaderboard_dict(
        cls, insight_dict: InsightDict
    ) -> InsightLeaderboardDict:
        print(type(insight_dict), insight_dict)
        return {
            "name": insight_dict["name"],
            "year": insight_dict["year"],
            "data": [{"value": x[0], "team_keys": x[1]} for x in insight_dict["data"]],
        }
