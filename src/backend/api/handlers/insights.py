...
from backend.common.queries.dict_converters.insight_converter import InsightConverter
...
def insights_leaderboards_all() -> Response:
    ...
    for future in futures:
        # Morph the json blob into the newly desired shape in the API so as not to break old py3 templates
        # todo(tervay): change leaderboards to save in this format in the db once py3 templates are removed
        insights.extend(
            [
                InsightConverter.insight_dict_to_api_leaderboard_dict(result)
                for result in future.get_result()
            ]
        )
