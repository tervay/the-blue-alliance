...
CounterDictType = (
    Dict[Any, int | float]
    | Dict[TeamKey, int | float]
    | Dict[EventKey, int | float]
    | Dict[MatchKey, int | float]
    | DefaultDict[Any, int | float]
    | DefaultDict[TeamKey, int | float]
    | DefaultDict[EventKey, int | float]
    | DefaultDict[MatchKey, int | float]
)
    ...
    def _calculate_leaderboard_highest_median_score_by_event(
        ...
        for _, week_events in week_event_matches:
            for event, matches in week_events:
                for match in matches:
                    if match.has_been_played:
                        scores[event.key.id()].append(match.alliances["red"]["score"])
                        scores[event.key.id()].append(match.alliances["blue"]["score"])
